...
- **Completed**: Spec com 8 user stories (7 P1 + 1 P2/E2E), 18 requirement IDs (CAT-01..07, TXN-01..11), edge cases, dimensions sweep e closure gate. Usuário revisou as 14 áreas cinzentas em 2026-07-18 (1ª revisão) e fez uma 2ª revisão na mesma data — todas as assumptions confirmadas. Decisões do usuário vs. defaults do agente: descrição da transação OPCIONAL (≤140 quando presente, "—" na listagem); paginação NUMERADA (páginas de 20); formulário de transação em MODAL; EXCLUSÃO de transações (confirmação simples + hard delete); EDIÇÃO de transações (mesmo modal, mesmas validações — "edição é essencial no MVP"); EXCLUSÃO de categoria personalizada (bloqueada em uso; sem uso exige digitar "excluir permanentemente" + aviso de irreversibilidade; FK RESTRICT; padrão não-excluível). Fora do escopo: edição/arquivamento de categorias, filtros/busca.
- **In-progress** (file:line): nenhum
- **Next step**: Design (feature Large — fase necessária): schema Prisma dos 2 módulos (Category global+personalizada tipada com FK RESTRICT, Transaction), APIs públicas de `categories` e `transactions`, contrato entre módulos (transactions → categories via index.ts, AD-010), estratégia do seed idempotente, componentes de UI (modal criar/editar, confirmação digitada, paginação numerada).
- **Blockers**: nenhum.
