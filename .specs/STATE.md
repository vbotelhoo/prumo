...
- **Feature**: categories-transactions (Roadmap item 3) — ✅ **concluída e mergeada em `main`** via PRs [#3](https://github.com/vbotelhoo/prumo/pull/3) (implementação) e [#4](https://github.com/vbotelhoo/prumo/pull/4) (ajuste de categorias padrão + seed). Os 5 checks de CI verdes em `main`. Ciclo Specify → Design → Tasks → Execute → Validate completo.
- **Resultado da Validate**: Verifier emitiu PASS em `.specs/features/categories-transactions/validation.md`. O PASS inicial (por inspeção de código, sem execução local de integração/E2E por causa de Node 20.15 vs. `engines` 24.18) foi depois confirmado por execução real no CI, após 4 gate failures encontrados e corrigidos — ver "Post-Verification Update" no validation.md.
- **CI findings (2026-07-19) — corrigidos**: (1) `getByText({selector})` inválido no Playwright; (2) CI rodava `prisma migrate deploy` mas nunca `prisma db seed`, e `tsx` (usado pelo seed) não era dependência declarada; (3) fixtures de integração usavam CPF sem checksum válido e `Headers()` vazio em vez do cookie de sessão real (toda action autenticada retornava Unauthorized); (4) **bug real de produção** — `TransactionsPageClient.handleModalSuccess` nunca chamava `router.refresh()`, então a lista não atualizava após criar/editar. 4 lições registradas em `.specs/lessons.json` (L-006 a L-009) para esses padrões.
- **In-progress** (file:line): nenhum
- **Next step**: **Roadmap item 4 — módulo `commitments`** (compras parceladas e financiamentos) é o próximo candidato a Specify. Módulo hoje é só scaffold (`src/modules/commitments/` com `.gitkeep`). Decisão AD-009 (parcelas materializadas) e AD-008 (dinheiro em centavos) já orientam o design.
- **Blockers**: nenhum.
- **Uncommitted files**: nenhum.
- **Branch**: `main` está em dia com `origin/main` incluindo categories-transactions. A branch `cursor/spec-categories-transactions-55cc` já foi mergeada; iniciar item 4 a partir de uma branch nova de `main`.
